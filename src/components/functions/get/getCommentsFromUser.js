...
export const getCommentsFromUser = async (userId) => {

  try {
    const response = await fetch(`${API_URL}${apiEndpoints.commentsByUser}/${userId}`); // Se usa la URL de la API y el endpoint de posts filtrando por userId
    if (!response.ok) {
