...
const FormEditarPost = ({ post, user, onCancel, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const resPost = await fetch(`http://localhost:3001/posts/${post._id}`, {
        method: "PUT",
      ...
      if (imagenes.length > 0 && editandoImagenes.length > 0) {
        for (let i = 0; i < Math.min(imagenes.length, editandoImagenes.length); i++) {
          ...
          const resImg = await fetch(`http://localhost:3001/archives/${editandoImagenes[i]._id}`, {
            method: "PUT",
            body: formData
          });
  ...
  return (
    <Form onSubmit={handleSubmit}>
      <Card className="w-100 bg-dark text-light my-4">
